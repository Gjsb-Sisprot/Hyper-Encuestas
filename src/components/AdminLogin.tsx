...
export default function AdminLogin({ onLoginSuccess, onCancelPublic }: AdminLoginProps) {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err?.message?.includes('Invalid login credentials') || err?.message?.includes('User not found')) {
        setErrorMsg('Credenciales no válidas. Puedes ingresar con credenciales de prueba o registrarte.')
      } else {
  ...
  const handleDemoAccess = () => {
    onLoginSuccess('admin@sisprot.com (Modo Demo)')
  }

  return (
    <div style={{
      minHeight: '100vh',
      background: 'radial-gradient(circle at 50% 30%, #0F1D38 0%, #060A14 100%)',
      display: 'flex',
      alignItems: 'center',
      ...
      <div style={{
        ...
        borderRadius: 24,
        padding: '36px 32px',
        boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.7), 0 0 40px rgba(0, 82, 255, 0.15)'
      }}>
        {/* Header Logo */}
        ...
        <form onSubmit={handleAuth} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          >
            {loading ? 'Autenticando...' : isRegistering ? 'Crear Cuenta Admin' : 'Iniciar Sesión Supabase'}
          </button>
        </form>

        <div style={{ marginTop: 20, paddingTop: 18, borderTop: '1px solid rgba(255,255,255,0.08)', textAlign: 'center', display: 'flex', flexDirection: 'column', gap: 12 }}>
          <button
            onClick={() => setIsRegistering(!isRegistering)}
            style={{ background: 'none', border: 'none', color: cyber, fontSize: 12, cursor: 'pointer', fontWeight: 600 }}
          >
            {isRegistering ? '¿Ya tienes cuenta? Inicia Sesión' : '¿Nuevo administrador? Registrarse'}
          </button>
          ...
          >
            ⚡ Ingreso Rápido Pre-Producción (Modo Demo Admin)
          </button>
